...
---
## Where are we now?
 1. a `context` will always be the first argument, if present
 1. we can successfully dump a stack and rebuild a context from it
 1. golang natively exposes only 4 implementations of the `Context` interface
 1. We have a fixed stack above our (TODO) implementation, with each of the standard context implementations
 1. We know `itab`s are fixed memory locations determined at compile time
